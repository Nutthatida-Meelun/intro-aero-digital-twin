...
1. If `Cm_alpha < 0` and the angle-of-attack disturbance is positive, `delta_Cm` should be `negative` because `negative slope`.
2. If `Cm_alpha > 0` and the angle-of-attack disturbance is positive, the response should be `destabilizing` because `a positive slope`.
3. If `Cm_alpha = 0`, changing angle of attack should `have no effect on the pitching moment(neutrally stable)`.
...
Angle conversion:
alpha_rad = 2.86 * pi / 180 = 0.0499164166
delta_alpha_rad = +2.00 * pi / 180 = 0.0349065850
